...
namespace Alchemy\Fields\Datalist;
...
class Field implements Field_Interface {
    function __construct() {
        ...
        add_filter( 'alch_get_datalist_option_html', array( $this, 'get_option_html' ), 10, 3 );
        add_filter( 'alch_sanitize_datalist_value', array( $this, 'sanitize_value' ) );
        add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_assets' ) );
        add_action( 'alch_prepare_datalist_value', array( $this, 'prepare_value' ), 10, 3 );
    ...
    function enqueue_assets() : void {
        wp_register_script(
            ...
            array(),
            filemtime( AlCHEMY_DIR_PATH . 'fields/datalist/vendor/select2/js/select2.full.min.js' ),
            true
        );

        $select2LanguageFile = sprintf( 'fields/datalist/vendor/select2/js/i18n/%1$s.js',
        ...
        wp_register_script(
            'alch_datalist_field',
            AlCHEMY_DIR_URL . 'fields/datalist/scripts.min.js',
            array( 'alch_admin_scripts', 'alch_select_2' ),
            filemtime( AlCHEMY_DIR_PATH . 'fields/datalist/scripts.min.js' ),
            true
        );

        wp_register_style(
            'alch_datalist_field',
            AlCHEMY_DIR_URL . 'fields/datalist/styles.min.css',
            array( 'alch_select_2' ),
            filemtime( AlCHEMY_DIR_PATH . 'fields/datalist/styles.min.css' )
        );

        wp_register_style(
            'alch_select_2',
            AlCHEMY_DIR_URL . 'fields/datalist/vendor/select2/css/select2.min.css',
            array(),
            filemtime( AlCHEMY_DIR_PATH . 'fields/datalist/vendor/select2/css/select2.min.css' )
        );

        wp_enqueue_script( 'alch_datalist_field' );
    ...
    function get_option_html( array $data, $savedValue, string $type ) : string {
        ...
        $html .= sprintf( '<select class="jsAlchemyDatalist" id="%1$s"%2$s data-alchemy="%3$s">',
            $data['id'],
        ...
        $html .= $this->get_datalist_options_html( $options, $savedValue );

        $html .= '</select>';

        $html .= '<button type="button" class="button button-secondary jsAlchemyDatalistClear"><span class="dashicons dashicons-trash"></span></button>';
    ...
    function prepare_value( $value, $id ) {
        $validValue = apply_filters( 'alch_prepared_datalist_value', $value );
    ...
    private function get_datalist_options_html( array $options, $savedValue ) : string {
        ...
        $optionsHTML .= join('', array_map( function( $option ) use( $savedValue ) {
            return sprintf(
                '<option value="%1$s"%3$s>%2$s</option>',
                esc_attr( $option['value'] ),
                $option['name'],
                selected( in_array( $option['value'], $savedValue ), true, false )
            );
        }, $options ));
